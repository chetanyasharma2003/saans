...
const generateInvalidSignature = () => {
  return 'invalid_signature_' + crypto.randomBytes(16).toString('hex');
};
...
async function setupTestUser() {
  try {
    ...
    const testEmail = `payment-verify-test-${Date.now()}@test.com`;
...
async function createTestOrder(planType = 'BASIC') {
  const response = await makeAuthRequest('POST', '/api/payments/create-order', { planType });
  if (response.success && response.data.data?.orderId) {
...
async function test1_ValidSignatureValidData() {
  ...
  try {
    ...
    if (response.success && response.status === 200) {
      ...
      addTestResult('TEST 1: Valid Signature + Valid Data', false, `Status: ${response.status}`);
      return false;
    }
  } catch (error) {
...
async function test2_ValidSignatureMissingPlanType() {
  ...
  try {
    ...
    const response = await makeAuthRequest('POST', '/api/payments/verify-payment', {
      ...
      razorpay_payment_id: paymentId,
      razorpay_signature: validSignature,
      // planType is missing intentionally
    });
...
// TEST 4: Missing required fields (order_id)
...
async function test5_IdempotencySamePayment() {
  ...
  try {
    ...
    // Second verification (should NOT create duplicate charge)
    ...
    // Check database for duplicate charges
    ...
    if (payments.length === 1) {
      ...
      log.error(`Multiple charges found: ${payments.length}`);
      addTestResult('TEST 5: Idempotency - Same Payment', false, `Found ${payments.length} charges`);
      return false;
    }
  } catch (error) {
    log.error(`Test failed: ${error.message}`);
...
// TEST 7: Signature with tampered order ID
async function test7_TamperedOrderId() {
  ...
  try {
    ...
    const response = await makeAuthRequest('POST', '/api/payments/verify-payment', {
      ...
      razorpay_payment_id: paymentId,
      razorpay_signature: validSignature, // Signature won't match tampered order ID
      planType: 'BASIC',
    });
...
// TEST 8: Signature with tampered payment ID
async function test8_TamperedPaymentId() {
  ...
  try {
    ...
    const response = await makeAuthRequest('POST', '/api/payments/verify-payment', {
      ...
      razorpay_signature: validSignature, // Signature won't match tampered payment ID
      planType: 'PREMIUM',
    });
...
// TEST 9: Unauthenticated request (no token)
...
async function test10_RateLimitingTest() {
  log.h2('TEST 10: Rate Limiting Protection');
  try {
    const orderId = await createTestOrder('BASIC');
    const basePaymentId = `pay_${Date.now()}`;
    const validSignature = generateValidSignature(orderId, basePaymentId);
    ...
    for (let i = 0; i < 5; i++) {
      const response = await makeAuthRequest('POST', '/api/payments/verify-payment', {
        razorpay_order_id: orderId,
        razorpay_payment_id: basePaymentId + `_${i}`,
        razorpay_signature: validSignature, // This will fail but tests rate limiting
        planType: 'BASIC',
      });

      if (response.status === 429) {
        rateLimitHit = true;
        log.success('Rate limit triggered after requests');
        break;
      }
    }

    if (rateLimitHit) {
      addTestResult('TEST 10: Rate Limiting Protection', true);
      return true;
    } else {
      log.warn('Rate limiting may not be active (check middleware configuration)');
      addTestResult('TEST 10: Rate Limiting Protection', false, 'No rate limit response');
      return false;
    }
  } catch (error) {
    log.error(`Test failed: ${error.message}`);
    addTestResult('TEST 10: Rate Limiting Protection', false, error.message);
    return false;
  }
}

// TEST 11: Database consistency check
async function test11_DatabaseConsistency() {
  log.h2('TEST 11: Database Consistency After Verification');
  try {
    const orderId = await createTestOrder('PREMIUM');
    ...
    if (!response.success) {
      ...
      addTestResult('TEST 11: Database Consistency', false, 'Verification failed');
      return false;
    }
    ...
    if (subscriptionExists && paymentRecorded && userPremiumStatus) {
      ...
      log.info(`Payment recorded: ${paymentRecorded}`);
      addTestResult('TEST 11: Database Consistency', true);
      return true;
    } else {
      const issues = [];
      ...
      addTestResult('TEST 11: Database Consistency', false, issues.join(', '));
      return false;
    }
  } catch (error) {
    log.error(`Test failed: ${error.message}`);
    addTestResult('TEST 11: Database Consistency', false, error.message);
    return false;
  }
}

// TEST 12: Concurrent verification requests
async function test12_ConcurrentRequests() {
  log.h2('TEST 12: Concurrent Verification Requests');
  try {
    const orderId = await createTestOrder('BASIC');
    ...
    for (let i = 0; i < 3; i++) {
      requests.push(
        makeAuthRequest('POST', '/api/payments/verify-payment', {
          razorpay_order_id: orderId,
          razorpay_payment_id: paymentId,
          razorpay_signature: validSignature,
          planType: 'BASIC',
    ...
    if (payments.length === 1 && successCount > 0) {
      log.success('Concurrent requests handled correctly (no duplicate charges)');
      log.info(`Successful responses: ${successCount}/3`);
      log.info(`Payments recorded: ${payments.length}`);
      addTestResult('TEST 12: Concurrent Requests', true);
      return true;
    } else {
      log.error(`Duplicate charges detected: ${payments.length}`);
      addTestResult('TEST 12: Concurrent Requests', false, `Charges: ${payments.length}, Successes: ${successCount}`);
      return false;
    }
  } catch (error) {
    log.error(`Test failed: ${error.message}`);
    addTestResult('TEST 12: Concurrent Requests', false, error.message);
    return false;
  }
}
...
async function runTests() {
  ...
  try {
    ...
    await test10_RateLimitingTest();
    await test11_DatabaseConsistency();
    await test12_ConcurrentRequests();
